...
class BeautyGAN():
    ...
    def input_read(self,sess):
        ...
        self.A_input_mask = np.zeros((max_images,4,img_height,img_width))
        self.B_input_mask = np.zeros((max_images,4,img_height,img_width))
            ...
            for i in range(max_images):
                ...
                if image_tensor.size==img_width*img_height*img_layer:
                    ...
                    if res!=None:
                        ...
                        self.A_input_mask[cur_A][3] = np.equal(res[3],255)
                        cur_A+=1
            ...
            for i in range(max_images):
                ...
                if image_tensor.size==img_width*img_height*img_layer:
                    ...
                    if res != None:
                        ...
                        self.B_input_mask[cur_B][3] = np.equal(res[3],255)
                        cur_B += 1
    ...
    def model_setup(self):
        ...
        self.input_A_mask_multigpu = tf.placeholder(tf.bool,[gpu_num,4,img_height,img_width],name="input_A_mask")
        self.input_B_mask_multigpu = tf.placeholder(tf.bool, [gpu_num, 4, img_height, img_width], name="input_B_mask")
            ...
            for gpu in range(gpu_num):
                with tf.device("/GPU:%d"%gpu):
                    with tf.name_scope("tower_%d"%gpu):
                        ...
                        temp_source2 = tf.cast((self.fake_A[0, :, :, 0] + 1) * 127.5, dtype=tf.float32)
                        temp_template = tf.cast((self.input_A_multigpu[gpu, :, :, 0] + 1) * 127.5, dtype=tf.float32)
                        temp_template2 = tf.cast((self.input_B_multigpu[gpu, :, :, 0] + 1) * 127.5, dtype=tf.float32)
                        histogram_loss_r_lip = self.face_loss_cal(temp_source, temp_template, self.input_A_mask_multigpu[gpu][0],
                                                                       self.input_A_mask_multigpu[gpu][0])
                        histogram_loss_r_eye = self.face_loss_cal(temp_source, temp_template, self.input_A_mask_multigpu[gpu][1],
                                                                       self.input_A_mask_multigpu[gpu][1])
                        histogram_loss_r_nose = self.face_loss_cal(temp_source, temp_template,self.input_A_mask_multigpu[gpu][2],
                                                                        self.input_A_mask_multigpu[gpu][2])
                        #histogram_loss_r_face = self.histogram_loss_cal(temp_source, temp_template2,self.input_A_mask_multigpu[gpu][3],
                        #                                                self.input_B_mask_multigpu[gpu][3])
                        
                        faceloss_r = self.face_loss_cal(temp_source, temp_template2,self.input_B_mask_multigpu[gpu][3],
                                                                        self.input_B_mask_multigpu[gpu][3])

                        histogram_loss_r_lip2 = self.face_loss_cal(temp_source2, temp_template2, self.input_B_mask_multigpu[gpu][0],
                                                                       self.input_B_mask_multigpu[gpu][0])
                        histogram_loss_r_eye2 = self.face_loss_cal(temp_source2, temp_template2, self.input_B_mask_multigpu[gpu][1],
                                                                       self.input_B_mask_multigpu[gpu][1])
                        histogram_loss_r_nose2 = self.face_loss_cal(temp_source2, temp_template2,self.input_B_mask_multigpu[gpu][2],
                                                                        self.input_B_mask_multigpu[gpu][2])
                        #histogram_loss_g_face = self.histogram_loss_cal(temp_source, temp_template2,self.input_A_mask_multigpu[gpu][3],
                        #                                                self.input_B_mask_multigpu[gpu][3])
                        faceloss_r2 = self.face_loss_cal(temp_source2, temp_template,self.input_A_mask_multigpu[gpu][3],
                                                                        self.input_A_mask_multigpu[gpu][3])

                        histogram_loss_r = histogram_loss_r_eye + histogram_loss_r_lip + histogram_loss_r_nose
                        histogram_loss_r2 = histogram_loss_r_eye2 + histogram_loss_r_lip2 + histogram_loss_r_nose2

                        temp_source = tf.cast((self.fake_B[0, :, :, 1] + 1) * 127.5, dtype=tf.float32)
                        temp_source2 = tf.cast((self.fake_A[0, :, :, 1] + 1) * 127.5, dtype=tf.float32)
                        temp_template = tf.cast((self.input_A_multigpu[gpu, :, :, 1] + 1) * 127.5, dtype=tf.float32)
                        temp_template2 = tf.cast((self.input_B_multigpu[gpu, :, :, 1] + 1) * 127.5, dtype=tf.float32)
                        histogram_loss_g_lip = self.face_loss_cal(temp_source, temp_template, self.input_A_mask_multigpu[gpu][0],
                                                                       self.input_A_mask_multigpu[gpu][0])
                        histogram_loss_g_eye = self.face_loss_cal(temp_source, temp_template, self.input_A_mask_multigpu[gpu][1],
                                                                       self.input_A_mask_multigpu[gpu][1])
                        histogram_loss_g_nose = self.face_loss_cal(temp_source, temp_template,self.input_A_mask_multigpu[gpu][2],
                                                                        self.input_A_mask_multigpu[gpu][2])
                        #histogram_loss_g_face = self.histogram_loss_cal(temp_source, temp_template2,self.input_A_mask_multigpu[gpu][3],
                        #                                                self.input_B_mask_multigpu[gpu][3])
                        faceloss_g = self.face_loss_cal(temp_source, temp_template2,self.input_B_mask_multigpu[gpu][3],
                                                                        self.input_B_mask_multigpu[gpu][3])

                        
                        histogram_loss_g_lip2 = self.face_loss_cal(temp_source2, temp_template2, self.input_B_mask_multigpu[gpu][0],
                                                                       self.input_B_mask_multigpu[gpu][0])
                        histogram_loss_g_eye2 = self.face_loss_cal(temp_source2, temp_template2, self.input_B_mask_multigpu[gpu][1],
                                                                       self.input_B_mask_multigpu[gpu][1])
                        histogram_loss_g_nose2 = self.face_loss_cal(temp_source2, temp_template2,self.input_B_mask_multigpu[gpu][2],
                                                                        self.input_B_mask_multigpu[gpu][2])
                        #histogram_loss_g_face = self.histogram_loss_cal(temp_source, temp_template2,self.input_A_mask_multigpu[gpu][3],
                        #                                                self.input_B_mask_multigpu[gpu][3])
                        faceloss_g2 = self.face_loss_cal(temp_source2, temp_template,self.input_A_mask_multigpu[gpu][3],
                                                                        self.input_A_mask_multigpu[gpu][3])


                        histogram_loss_g = histogram_loss_g_lip + histogram_loss_g_eye + histogram_loss_g_nose 
                        histogram_loss_g2 = histogram_loss_g_eye2 + histogram_loss_g_lip2 + histogram_loss_g_nose2

                        temp_source = tf.cast((self.fake_B[0, :, :, 2] + 1) * 127.5, dtype=tf.float32)
                        temp_source2 = tf.cast((self.fake_A[0, :, :, 2] + 1) * 127.5, dtype=tf.float32)
                        temp_template = tf.cast((self.input_A_multigpu[gpu, :, :, 2] + 1) * 127.5, dtype=tf.float32)
                        temp_template2 = tf.cast((self.input_B_multigpu[gpu, :, :, 2] + 1) * 127.5, dtype=tf.float32)
                        histogram_loss_b_lip = self.face_loss_cal(temp_source, temp_template, self.input_A_mask_multigpu[gpu][0],
                                                                       self.input_A_mask_multigpu[gpu][0])
                        histogram_loss_b_eye = self.face_loss_cal(temp_source, temp_template, self.input_A_mask_multigpu[gpu][1],
                                                                       self.input_A_mask_multigpu[gpu][1])
                        histogram_loss_b_nose = self.face_loss_cal(temp_source, temp_template,self.input_A_mask_multigpu[gpu][2],
                                                                        self.input_A_mask_multigpu[gpu][2])
                        ...
                        histogram_loss_b_lip2 = self.face_loss_cal(temp_source2, temp_template2, self.input_B_mask_multigpu[gpu][0],
                                                                       self.input_B_mask_multigpu[gpu][0])
                        histogram_loss_b_eye2 = self.face_loss_cal(temp_source2, temp_template2, self.input_B_mask_multigpu[gpu][1],
                                                                       self.input_B_mask_multigpu[gpu][1])
                        histogram_loss_b_nose2 = self.face_loss_cal(temp_source2, temp_template2,self.input_B_mask_multigpu[gpu][2],
                                                                        self.input_B_mask_multigpu[gpu][2])
                        #histogram_loss_g_face = self.histogram_loss_cal(temp_source, temp_template2,self.input_A_mask_multigpu[gpu][3],
                        #                                                self.input_B_mask_multigpu[gpu][3])
                        faceloss_b2 = self.face_loss_cal(temp_source2, temp_template,self.input_A_mask_multigpu[gpu][3],
                                                                        self.input_A_mask_multigpu[gpu][3])


                        histogram_loss_b = histogram_loss_b_lip + histogram_loss_b_nose + histogram_loss_b_eye 
                        histogram_loss_b2 = histogram_loss_b_eye2 + histogram_loss_b_lip2 + histogram_loss_b_nose2
                        
                        innerface_loss_B = histogram_loss_r + histogram_loss_g + histogram_loss_b
                        innerface_loss_A = histogram_loss_r2 + histogram_loss_g2 + histogram_loss_b2
    ...
    def face_loss_cal(self,source,template,source_mask,template_mask):
        ...
        template = tf.subtract(tf.divide(template,127.5),1)
        
        return tf.reduce_mean(tf.abs(tf.subtract(template,source)))
    ...
    def loss_cal(self):
        ...
        histogram_loss_r_nose = self.histogram_loss_cal(temp_source,temp_template, self.input_A_mask[2],self.input_B_mask[2])
        histogram_loss_r_face = self.histogram_loss_cal(temp_source,temp_template, self.input_A_mask[3],self.input_B_mask[3])
        histogram_loss_r = histogram_loss_r_lip+histogram_loss_r_eye+histogram_loss_r_nose+histogram_loss_r_face
        ...
        histogram_loss_g_nose = self.histogram_loss_cal(temp_source,temp_template, self.input_A_mask[2],self.input_B_mask[2])
        histogram_loss_g_face = self.histogram_loss_cal(temp_source,temp_template,self.input_A_mask[3],self.input_B_mask[3])
        histogram_loss_g = histogram_loss_g_lip+histogram_loss_g_eye+histogram_loss_g_nose+histogram_loss_g_face
        ...
        histogram_loss_b_nose = self.histogram_loss_cal(temp_source,temp_template, self.input_A_mask[2],self.input_B_mask[2])
        histogram_loss_b_face = self.histogram_loss_cal(temp_source,temp_template,self.input_A_mask[3],self.input_B_mask[3])
        histogram_loss_b = histogram_loss_b_lip+histogram_loss_b_eye+histogram_loss_b_nose+histogram_loss_b_face
        ...
        self.g_summary = tf.summary.merge([
            self.disc_A_loss_sum,self.disc_B_loss_sum,self.cyc_loss_sum,self.makeup_loss_sum,self.g_loss_sum,
        ],"g_summary")
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(sess.run(self.global_step),200):
                ...
                for ptr in range(0,self.train_num-(gpu_num-1),gpu_num):
                    print("epoch"+str(epoch)+" iteration ",ptr) 
